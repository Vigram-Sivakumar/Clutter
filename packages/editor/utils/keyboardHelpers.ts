...
function findPreviousBlockForIndent(
  ...
  log('findPreviousBlockForIndent DEBUG', {
    ...
    currentLevel,
    candidates,
    foundBlockId: found ? ((found as any).node.attrs.blockId as string).substring(0, 8) : 'null',
...
export function indentBlock(
  ...
  const prevBlock = findPreviousBlockForIndent(state, blockPos, currentLevel, currentBlockId);
  
  if (!prevBlock) {
    log('INDENT BLOCKED: No valid parent found', {
      reason: totalBlocks === 1 ? 'Only 1 block in document' : 'No block at same/lower level before current position',
      currentLevel,
    });
    return false;
  }
...
export function outdentBlock(
  ...
  state.doc.descendants((node: any, pos: number) => {
    if (node.attrs?.blockId && 
        ...
        pos > blockPos) { // After current block in document order
...
export function createWrapperEnterHandler(
  ...
  return ({ editor }: { editor: Editor }) => {
    ...
    if (isAtEnd) {
      ...
      tr.insert(after, nodeType.create({
        ...wrapperNode!.attrs,
        blockId: crypto.randomUUID(),
      }));
...
export function createWrapperBackspaceHandler(
  ...
  return ({ editor }: { editor: Editor }) => {
    const context =
      BackspaceRules.getWrapperBlockBackspaceContext(editor, wrapperType);
