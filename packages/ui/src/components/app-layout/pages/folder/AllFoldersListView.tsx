...
interface AllFoldersListViewProps {
  ...
  onCreateNote: (_folderId: string) => void;
  onCreateFolder: () => void;
}
...
export const AllFoldersListView = ({
  ...
  const allFolders = useMemo(() => {
    const activeFolderIds = new Set(
      folders.filter((f) => !f.deletedAt).map((f) => f.id)
    );

    // Cluttered notes count (all notes, including empty ones)
    const clutteredNotes = notes.filter(
      (note) =>
        !note.deletedAt &&
    ...
    const rootFolders = folders
      .filter(
        (f) => !f.deletedAt && !f.parentId && f.id !== DAILY_NOTES_FOLDER_ID // Exclude Daily Notes system folder
      )
      .sort(
        (a, b) =>
