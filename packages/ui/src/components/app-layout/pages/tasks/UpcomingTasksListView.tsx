...
export const UpcomingTasksListView = ({
  onTaskClick,
}: UpcomingTasksListViewProps) => {
  const { notes, updateNoteContent } = useNotesStore();
  const todayDateString = useMemo(() => getTodayDateString(), []);

  // Get all upcoming tasks using centralized logic
  const upcomingTasks = useMemo(() => {
    ...
    const { upcoming } = categorizeTasks(allTasks, todayDateString);
    return sortTasksByDateAndCreation(upcoming);
  ...
  return (
    <>
      <PageTitleSection
        variant="folder"
        folderName={SECTIONS.upcoming.label}
        staticIcon={renderIcon(
          SECTIONS.upcoming.iconName,
      ...
      <ListViewLayout
        sections={sections}
        emptyState={SECTIONS.upcoming.emptyMessage}
      />
    </>
  );
};
