...
export const DeletedItemsListView = ({
  ...
  const permanentlyDeleteTag = useTagsStore(
    (state) => state.permanentlyDeleteTag
  );
  ...
  const deletedTags = useMemo(() => {
    const tags = Object.values(tagMetadata).filter(
  ...
  const getNoteLocation = useCallback(
    (note: any) => {
  ...
  const getFolderLocation = useCallback(
    (folder: any) => {
    ...
    () =>
      deletedNotes
        ...
        .map((note) => {
          ...
          const parentDeleted =
            note.folderId &&
            folders.find((f) => f.id === note.folderId)?.deletedAt;
    ...
    () =>
      deletedFolders.map((folder) => {
        ...
        const notesInFolder = notes.filter(
          (n) => n.deletedAt && n.folderId === folder.id
        );
        const subfoldersInFolder = folders.filter(
  ...
  const tagListItems = useMemo(() => {
    const items = deletedTags.map((tag) => {
      ...
      const noteCount = notes.filter(
        (n) =>
          !n.deletedAt &&
          n.tags.some((t) => t.toLowerCase() === tag.name.toLowerCase())
      ).length;

      const folderCount = folders.filter(
        (f) =>
          !f.deletedAt &&
          f.tags?.some((t) => t.toLowerCase() === tag.name.toLowerCase())
      ).length;
