...
export const TagFilteredNotesView = ({ 
  ...
  const [description, setDescription] = useState(tagMetadata?.description || '');
  const [showDescriptionInput, setShowDescriptionInput] = useState(false);
  const [descriptionVisible, setDescriptionVisible] = useState(tagMetadata?.descriptionVisible ?? true);
  ...
  useEffect(() => {
    if (tagMetadata) {
      ...
      // Create metadata entry for this tag if it doesn't exist
      // Store the hash-based color so it persists through renames
      upsertTagMetadata(tag, '', true, false, getTagColor(tag));
    }
  }, [tag, tagMetadata, upsertTagMetadata]);
