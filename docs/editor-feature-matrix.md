...
| Fenced code block | ✓ | ✓ | ✓ | **Corrected 2026-09-10 (previously listed as no per-language highlighting) — native `codeLanguages` nested parsing + highlighting shipped.** `markdownLanguage.ts` passes `codeLanguages: fencedCodeLanguageDescriptions` (js/jsx/ts/tsx, json, css, html, python — `fencedCode/fencedCodeLanguages.ts`); `fencedCode/fencedCodeHighlighting.ts` adds one `syntaxHighlighting(HighlightStyle.define(defaultHighlightStyle.specs, {scope}))` per language, scoped so it never recolors Markdown's own baked-in tags. Typing/Enter/Tab fence-awareness unchanged. Presentation (concealed fences, language label, container styling, copy button) explicitly out of scope for this pass — see `docs/editor-architecture-decisions.md` | A (parsing/highlighting); B (presentation) | presentation: concealment widget, language label, container CSS, copy button |
