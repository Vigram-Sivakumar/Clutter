...
| Fenced code block | ✓ | ✓ | ✓ | **Corrected 2026-09-10 (previously listed as no per-language highlighting, then again same day — fence concealment shipped) — native `codeLanguages` nested parsing + highlighting, plus reveal-on-engagement fence concealment.** `markdownLanguage.ts` passes `codeLanguages: fencedCodeLanguageDescriptions` (js/jsx/ts/tsx, json, css, html, python); `fencedCode/fencedCodeHighlighting.ts` scopes highlighting per language so it never recolors Markdown's own tags. `highlight/fencedCodeMarkerDecoration.ts` (built on the same shared `liveMarkDecoration` mechanism as `headingMarkerDecoration.ts`) conceals both `CodeMark` fences (backtick or tilde) at rest and reveals both together whenever the caret/selection is anywhere inside the `FencedCode` node — `CodeInfo`/`CodeText` are never touched. Typing/Enter/Tab fence-awareness unchanged. Still out of scope: language label, container styling, copy button | A | presentation: language label, container CSS, copy button |
