...
| Fenced code block | ✓ | ✓ | ✓ | **Corrected 2026-09-10 (fourth pass same day — per-block independence, right-aligned header, canonical labels) — feature-complete fenced-code presentation.** Native `codeLanguages` nested parsing/highlighting and fence-marker reveal-on-engagement unchanged. `highlight/fencedCodeBlockLineDecoration.ts` now derives each line's `--first`/`--last` modifier from its *owning* `FencedCode` node's own `.from`/`.to`, not DOM sibling adjacency — the earlier one-flat-class-plus-adjacency technique incorrectly merged two back-to-back fenced blocks with no blank line between them into one container (legal Markdown, confirmed reproduced); each block now always renders as its own independent card. The language label (`highlight/fencedCodeLanguageLabelDecoration.ts`) is now positioned via CSS `position: absolute` immediately left of the Copy button (both right-aligned), rather than in its natural in-flow position; its text is read directly from the matched `LanguageDescription.name` (`fencedCode/fencedCodeLanguages.ts`'s own `name` fields are now the canonical display casing, e.g. `JavaScript`/`TypeScript`/`Python`) — no second alias/display-name table (`fencedCode/fencedCodeLanguageLabel.ts` was simplified accordingly). JSX/TSX display as `JavaScript`/`TypeScript` respectively, since neither has a distinct `LanguageDescription` entry of its own — a registry-shape fact, not a display-layer special case. No wrapper element anywhere; verified directly via DOM inspection (three adjacent blocks, zero blank lines, three independent `--first`/`--last` pairs) in the real app | A | — |
