...
| Fenced code block | ✓ | ✓ | ✓ | **Corrected 2026-09-10 (third pass same day — container styling, friendly language label, and Copy button shipped) — feature-complete fenced-code presentation.** Native `codeLanguages` nested parsing/highlighting (`fencedCode/fencedCodeHighlighting.ts`) and fence-marker reveal-on-engagement (`highlight/fencedCodeMarkerDecoration.ts`) unchanged from the two earlier passes. New: `highlight/fencedCodeBlockLineDecoration.ts` gives every physical line of a `FencedCode` node one flat `cm-code-block-line` class, with continuous background/border/rounded-corners derived purely via CSS sibling adjacency — the same technique `blockquoteLineDecoration.ts`'s own corner-rounding already uses, no begin/middle/end class triple. `highlight/fencedCodeLanguageLabelDecoration.ts` replaces raw `CodeInfo` with a friendly name (`js`→`JavaScript`, via `fencedCode/fencedCodeLanguageLabel.ts`) while the enclosing block is at rest, reverting to raw text when engaged — keyed to the same `isTokenEngaged(FencedCode)` query the fence markers use, not the participant map (whose engagement is hardwired to the decorated node's own range — a mismatch for a label whose scope is its *enclosing* node, the same issue `inlineLivePreviewRegion.ts` already documents for heading content). `highlight/fencedCodeCopyButtonDecoration.ts` inserts (never replaces) a `.cm-media-control`-styled Copy button at the end of the opening line, copying exactly the `CodeText` node's content (re-resolved at click time, never a captured/stale value) via `navigator.clipboard.writeText` — the same interactive-widget pattern (`mousedown`/`click` `preventDefault`+`stopPropagation`, `ignoreEvent(): false`) `NoteEmbedWidget.ts`'s own buttons already use. No wrapper element anywhere; verified directly via DOM inspection in the real app | A | — |
