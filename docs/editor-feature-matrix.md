...
| Fenced code block | ✓ | ✓ | ✓ | **Corrected 2026-09-10 (fifth pass same day — container migrated to native `EditorView.blockWrappers`).** `highlight/fencedCodeBlockWrapper.ts` replaces the `Decoration.line`-based container entirely: CM6's own `blockWrappers` API wraps every `.cm-line` a `FencedCode` node owns in a real, CM6-created `<div class="cm-code-block">` parent — confirmed via DOM inspection (`<div class="cm-code-block"><div class="cm-line">…`), both in tests and the real app. `.cm-line` now carries zero fenced-code classes or container styling of any kind (not just margin) — a hard rule going forward, following a dedicated architecture investigation. Two back-to-back blocks (no blank line) get two independent wrappers structurally, not via adjacency logic. Native `codeLanguages` highlighting, fence-marker reveal-on-engagement, the language label, and the Copy button are all unchanged in mechanism. **Known, separate, pre-existing issue found during this pass's verification** (not introduced or fixed here): a mouse drag ending near a block's closing fence over-extends the selection to consume the marker — isolated to `fencedCodeMarkerDecoration.ts`'s `liveMarkSelectionSnap.ts` reuse, not the container; mid-block drags away from either fence work correctly | A (container); B (drag-near-closing-fence over-extension) | Investigate `liveMarkSelectionSnap.ts`'s `'node-range'` snap-to-boundary logic for pointer-drag interaction with multi-line constructs |
